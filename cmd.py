...
async def skills(ctx):
    quotes = [
        "**يلعبها يعرف شطرنج و هو مايعرف ماالشعر**",
        "**غشاش و يلعب بالانجين**",
        "**الشطرنج خاطيك خوياسمحلي**",
        "**راك سور بلي نت تلعب شطرنج؟**",
        "**كي يكون خاسر يضحي باش كي تربحو يقولك انا لي غامرت و ضحيتلك كون مربحتينش**",
        "**كون تعرف واش هوالسكيور نحبس شطرنج انا**",
        "**كل مرة يخسر يحل كونط جديد ويقلهم جاميخسرت**",
        "**خاطيه و علابالوبلي خاطيه ومقادر روحوا**",
        "**لي محبسين فالبريمارميعرفوش للشطرنج**",
        "**جامي خسر ضد بطل العالم**",
        "**هارب بزاف**",
        "**يلعبلك افتتاح كيالشكبي مبعد يقعرك, حداري من هدا الاخير**",
        "**جايب روحوا ومع شوية اونثرانمون قادريتعلم كيفاه يتعلم الحصان**",
        "**معلومة عن هدااللاعب: يعمل مع فريق ماغنش كالرسن**",
        "**سريع البديهة و سريع البرندرز**",
        "**ياخو نت ماشي تلعب الضامة؟**",
        "**روح دير فيها بارتية و ارواح نحللوها و نحكمو**",
        "**كون تشارك فالثورنوات تديهالهم**",
        "**يتعلم زوج افتتاحيات باش يحقر على الكهول فالقهوة**",
        "**يا المتوحد حبس متهدر مع بوت**",
        "**ملاعبي صحبنا هدا**",
        "**واعر فالدفاع**",
        "**يهاجم بزاف و كون يلقى ثغرة خلاصت عليك**",
        "**خلونا من الشطرنج ارواح بريفي ازين**",
        "**يراطي شاكميت مبعد يقولك راني شفتها لكن خليتك تكمل تلعب**",
        "** السيرفر كامل يخاف منو وهو يلعب بالانجين**",
        "**متهدرش معايي راني نلعب**",
        "**اصبر نخلص الغيم ونرد عليك رابيد**",
        "**ارواح تقابل اكتب !play**",
        "**تعلم الشطرنج من فيديوهات تع كش في 3 نقلات وجاي يوري فوجهو**",
    ]
...
@client.event
async def on_ready():
    # Check if the join_requests.json file exists
    if os.path.isfile("join_requests.json"):
        # Load the join requests from the existing file
        with open("join_requests.json", "r") as file:
            join_requests = json.load(file)

    print("Bot is ready")

@client.command()
async def join(ctx, lichess, tournament):
    try:
        user = await client.fetch_user(765595488632045601)
        author = ctx.message.author
        await user.send(f"{author.name} just participated in {tournament}, their lichess account is {lichess}")
        await ctx.send(f"{author.name}, your join request has been sent successfully!")

        if tournament in join_requests:
            join_requests[tournament].append({"player": author.name, "lichess": lichess})
        else:
            join_requests[tournament] = [{"player": author.name, "lichess": lichess}]

        # Save the join requests to the JSON file
        with open("join_requests.json", "w") as file:
            json.dump(join_requests, file)

    except discord.NotFound:
        await ctx.send("Could not send the request")

@client.command()
async def send_requests(ctx):
    try:
        # Load the join requests from the JSON file
        with open("join_requests.json", "r") as file:
            join_requests = json.load(file)

        # Check if there are any join requests
        if join_requests:
            # Iterate over each tournament entry in the join_requests dictionary
            for tournament, requests in join_requests.items():
                message = f"Join requests for {tournament}:\n"
                for request in requests:
                    player = request["player"]
                    lichess = request["lichess"]
                    message += f"Player: {player}, Lichess: {lichess}\n"

                # Send the join requests message
                await ctx.send(message)
        else:
            await ctx.send("No join requests found.")

    except FileNotFoundError:
        await ctx.send("No join requests found.")

    except json.JSONDecodeError:
        await ctx.send("Invalid join_requests.json file.")
...
@client.command()
async def clearrequests(ctx):
    try:
        # Clear the join_requests dictionary
        join_requests.clear()

        # Save the empty join requests to the JSON file
        with open("join_requests.json", "w") as file:
            json.dump(join_requests, file)

        await ctx.send("Join requests cleared.")
...
async def showmatches(ctx):
    try:
        ...
        if join_requests:
            ...
            tournaments = list(join_requests.keys())

            # Iterate over each tournament
            for tournament in tournaments:
                requests = join_requests[tournament]
                ...
                for i in range(0, len(requests), 2):
                    if i + 1 < len(requests):
                        player1 = requests[i]
                        player2 = requests[i + 1]
                        matches.append((player1, player2))

                # Format and send the match pairings message
                message = f"Match pairings for **{tournament}**:\n"
